...
exports = function(zipcode) {
  yelp.getFoodByZip(zipcode)
    .then(function(data){
        ...
        pgClient.connect(pgConConfig, function(err) {
          if (err){
          ...
          data.map(function(restaurant) {
            ...
            pgClient.query(sqlInsertRestaurants, [listing.name, listing.description, listing.phone, listing.streetAddress, listing.city, listing.state, listing.zipCode, listing.imageUrl, listing.eat24Url, listing.yelpRating, listing.yelpId, listing.cuisines], function (err, result){
              if (err){                 
